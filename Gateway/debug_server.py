...
WEBCAM_INDEX  = 0
INFERENCE_W   = 640          # inference resolution width
INFERENCE_H   = 480          # inference resolution height
CONF_THRESH   = 0.40
IOU_THRESH    = 0.45
FPS_LIMIT     = 10           # max inference FPS (cap to not fry CPU)
WS_PORT       = 9001
HTTP_PORT     = 9002
SLEEP_THRESH  = 15           # frames with no eyes before sleeping event
...
def inference_loop():
    ...
    log.info("Webcam opened (index=%d) at %dx%d", WEBCAM_INDEX, INFERENCE_W, INFERENCE_H)
    ...
    while True:
        ...
        for det in detections:
            ...
            cv2.putText(annotated, txt, (x1+2, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
        ...
        cv2.putText(annotated, overlay_txt, (8, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.55, ec_bgr, 1)
        fps_txt = f"FPS:{fps_avg:.1f}  #{frame_id}"
        cv2.putText(annotated, fps_txt, (w - 130, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)
        ...
        if frame_id % FPS_LIMIT == 0:
            log.info("frame=%d  fps=%.1f  event=%s  dets=%d",
                     frame_id, fps_avg, event, len(detections))
