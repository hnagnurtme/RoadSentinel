                        ...
                        { loadingHistory ? (
                            ...
                            <div className="flex flex-col divide-y divide-surface-container-high max-h-[400px] overflow-y-auto">
                                { sessions.map( session => {
                                    ...
                                    return (
                                        <div key={ session.id } className={`p-4 flex flex-col gap-1.5 transition-colors ${isActive ? "bg-emerald-500/5" : ""}`}>
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm font-bold flex items-center gap-1.5">
                                                    <span className="w-2 h-2 rounded-full bg-primary" />
                                                    Clock In: { new Date( session.started_at ).toLocaleString( 'en-GB', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' } ) }
                                                </span>
                                                { isActive ? (
                                            ...
                                            <div className="flex items-center justify-between text-xs text-secondary pl-3.5">
                                                <span>
                                                    { isActive ? (
                                                        "Active shift in progress"
                                                    ) : (
                                                        `Clock Out: ${new Date( session.ended_at || "" ).toLocaleString( 'en-GB', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' } )}`
                                                    ) }
                                                </span>
